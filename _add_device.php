...
if ($test->is_admin) {
  ...
  $sql_displ_count = "SELECT rtu_type_name, rtu_type_display_count, analog_start, analog_end FROM rtu_types WHERE rtu_type_name = '{$type}'";
  $types = $mysqli->query($sql_displ_count);
  $type_row = $types->fetch_object();

  $sql = "INSERT INTO rtu_list (rtu_id, rtu_ip, rtu_port, type, display_count, analog_start, analog_end)
          VALUES ({$rtu_id}, '{$rtu_ip}', {$port}, '{$type}',
          {$type_row->rtu_type_display_count}, {$type_row->analog_start}, {$type_row->analog_end})";
